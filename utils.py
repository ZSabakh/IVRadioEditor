...
def install_ffmpeg(parent_window=None):
    ...
    if not check_ffmpeg():
        reply = QMessageBox.question(
            parent_window,
            'FFmpeg Required',
            'FFmpeg is required but not found on your system. Would you like to install it now?',
        ...
        if reply == QMessageBox.Yes:
            if not is_admin():
                admin_reply = QMessageBox.question(
                    ...
                    'Installing FFmpeg requires administrator privileges. Would you like to restart the application as administrator?',
                    QMessageBox.Yes | QMessageBox.No
                )
                ...
                if admin_reply == QMessageBox.Yes:
                    if restart_as_admin():
                        ...
                        QMessageBox.critical(
                            parent_window,
                            'Error',
                            'Failed to restart as administrator. Please run the application as administrator manually.',
                            QMessageBox.Ok
                        )
                return False
            ...
            try:
                ...
                if set_system_path_env(bin_path):
                    ...
                    QMessageBox.critical(
                        parent_window,
                        'Error',
                        'Failed to set system PATH. Please install FFmpeg manually.',
                        QMessageBox.Ok
                    )
                    return False
            ...
            QMessageBox.warning(
                parent_window,
                'FFmpeg Required',
                'FFmpeg is required for audio processing. The application may not work correctly without it.',
                QMessageBox.Ok
            )
            return False
    return True
